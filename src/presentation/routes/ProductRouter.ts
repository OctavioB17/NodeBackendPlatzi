...
import { createProductSchema, getMultipleProductSchema, getProductSchema, getProductSchemaByName, getProductWPaginationAndPriceOperators, updatePhotosSchema, updateStockSchema } from "../../infraestructure/validators/ProductSchema";
...
router.delete('/delete', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('USER'), validatorHandler(getMultipleProductSchema, 'body'), (req, res, next) => productController.deleteProductController(req, res, next));
...
router.get('/get-all/user/:id', passport.authenticate('jwt', { session: false, failWithError: false }), validatorHandler(getProductSchema, 'params'), validatorHandler(getProductWPaginationAndPriceOperators, 'query'), (req, res, next) => productController.findAllByUserIdController(req, res, next));
...
router.patch('/update/pause', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('USER'), validatorHandler(getMultipleProductSchema, 'body'), (req, res, next) => productController.toggleProductPauseController(req, res, next));
router.patch('/update/:id', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('USER'), upload.array('image'), validatorHandler(getProductSchema, 'params'), (req, res, next) => productController.updateProductController(req, res, next));
...
router.delete('/delete/:productId/photo/:photoId', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('USER'), (req, res, next) => productController.deleteProductPhotoController(req, res, next));
