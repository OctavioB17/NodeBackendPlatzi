...
router.get('/find/id/:id', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('MODERATOR'), validatorHandler(getOrderSchema, 'params'), (req, res, next) => ordersController.findByIdController(req, res, next));
router.get('/find/user-id/:userId', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('MODERATOR'), validatorHandler(getOrderSchemaByUserId, 'params'), validatorHandler(paginationSchema, 'query'), (req, res, next) => ordersController.findByUserIdController(req, res, next));
router.get('/find/my-orders', passport.authenticate('jwt', { session: false }), checkRoleMiddelware('USER'), validatorHandler(paginationSchema, 'query'), (req, res, next) => ordersController.findUserOrdersController(req, res, next))
